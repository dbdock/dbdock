...
function makeJob(id: string, nextAttemptAt = '2000-01-01T00:00:00.000Z'): PendingJob {
  return {
    id,
    idempotencyKey: 'k',
    baseRevision: 0,
...
describe('queue', () => {
  ...
  it('increments attempts and backs off on failure', () => {
    ...
    expect(new Date(state.pending[0].nextAttemptAt).getTime()).toBeGreaterThan(1000);
  });

  it('moves a job to dead-letter at max attempts', () => {
    let state = defaultLocalState('p');
    state = enqueue(state, { ...makeJob('a'), attempts: DEFAULT_MAX_ATTEMPTS - 1 });
    state = markFailed(state, 'a', DEFAULT_MAX_ATTEMPTS, 1000);
    expect(state.pending).toHaveLength(0);
